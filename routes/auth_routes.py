...
def update_user():
    try:
        data = request.get_json()
        original_email = data.get('originalEmail')  # Should be anon_{session_id}@quickcampaigns.io
        updated_email = data.get('updatedEmail').strip().lower()
        new_username = data.get('username')
        new_password = data.get('password')

        if not original_email or not updated_email or not new_username or not new_password:
